...
type_counts = inn.groupby('Bowling Type').agg(Num=('Runs','count'))
inn = inn.groupby(['Bowling Type','How Out']).agg(Num=('Runs','count'))
inn['Percentage'] = round(100*(inn['Num']/type_counts["Num"]),1)
inn = inn.reset_index()
print(inn)
fig7 = px.bar(inn,x='Bowling Type',y='Percentage',color='How Out',color_discrete_map=dismissal_type_dict,labels={'Percentage':'%'})
#fig7.update_layout(showlegend=False)
fig7.update_yaxes(range=[0,100])
fig7.update_layout(title=f'Dismissal Type by Bowling Type',template='none')
with col5:  
